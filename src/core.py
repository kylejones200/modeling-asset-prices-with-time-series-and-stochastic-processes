...
def simulate_wiener_process(
    n_steps: int = 1000, T: float = 1.0, seed: int = None
) -> np.ndarray:
...
def plot_wiener_process(
    W: np.ndarray, t: np.ndarray, output_path: Path, plot: bool = False
):
    """Plot Wiener process"""
    if plot:
...
def plot_gbm_simulation(
    t: np.ndarray, S: np.ndarray, output_path: Path, plot: bool = False
):
